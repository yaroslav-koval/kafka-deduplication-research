...
func (ch *ClickHouse) Conn() *sql.DB {
	return ch.conn
}
...
func (ch *ClickHouse) init() error {
	...
	if ch.config.MigrationsTableEngine == "" {
...
func (ch *ClickHouse) Version() (v int, check bool, err error) {
	var (
		d     uint8
		query = "SELECT version, dirty FROM `" + ch.config.MigrationsTable + "` ORDER BY sequence DESC LIMIT 1"
	)

	if err := ch.conn.QueryRow(query).Scan(&v, &d); err != nil {
...
func (ch *ClickHouse) SetVersion(version int, dirty bool) error {
	...
	_, err = stm.Exec(int64(version), boolF(dirty), time.Now().UnixNano())
	if err != nil {
		return &database.Error{OrigErr: err, Query: []byte(query)}
	}
