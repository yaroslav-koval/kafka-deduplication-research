...
func AbortErr(c *gin.Context, statusCode int, message ...string) {
	msgErr := StatusMessage(statusCode)
	if len(message) > 0 {
		msgErr = message[0]
	}

	c.AbortWithStatusJSON(statusCode,
		cerror.BuildErrorResponse(cerror.NewF(c, cerror.KindFromHTTPCode(statusCode), msgErr).LogError()))
}

func NewError(c *gin.Context, code int, message ...string) error {
	msgErr := StatusMessage(code)
	if len(message) > 0 {
		msgErr = message[0]
	}

	return cerror.NewF(c, cerror.KindFromHTTPCode(code), msgErr).
		LogError()
}

func AbortWithError(c *gin.Context, e error) {
	c.AbortWithStatusJSON(cerror.ErrKind(e).HTTPCode(), cerror.BuildErrorResponse(e))
}
